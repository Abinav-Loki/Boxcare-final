...
function SignInContent() {
  ...
  useEffect(() => {
    ...
    if (error) {
      if (error === "oauth_cancelled") {
        setMessage({ type: "info", text: "Google sign-in was cancelled." });
      } else if (error === "not_configured") {
        setMessage({
          type: "info",
          text: "Google Sign-In is not configured yet. Please add your GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env.local to enable live Google authentication.",
        });
      } else if (error === "unverified_email") {
        setMessage({
          type: "error",
          text: "Your Google email address is not verified by Google. Please verify it or sign up with a password.",
        });
      } else if (error === "conflict_error") {
        setMessage({
          type: "error",
          text: "A conflict occurred while authenticating with Google. Please sign in using your email and password.",
        });
        ...
        setMessage({
          type: "error",
          text: "Unable to complete Google sign-in. Please try again or use your password.",
        });
      }
    }
  }, [searchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      setMessage({
        type: "success",
        text: "Sign In successful! Redirecting to your customer profile...",
      });
      setTimeout(() => {
        router.push("/profile");
      }, 700);
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    const res = await customerForgotPasswordAction({ email: forgotEmail });
        ...
        <div style={{ padding: "48px 40px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <div style={{ marginBottom: "20px" }}>
            <a
              ...
              onMouseEnter={(e) => {
                (e.currentTarget as HTMLElement).style.backgroundColor = "#F9F6F0";
                (e.currentTarget as HTMLElement).style.borderColor = "#D68A45";
              }}
              onMouseLeave={(e) => {
                (e.currentTarget as HTMLElement).style.backgroundColor = "#FFFFFF";
                (e.currentTarget as HTMLElement).style.borderColor = "#E2D7CC";
              }}
            >
              <GoogleIcon />
            ...
            {!forgotSubmitted ? (
              ...
              <div style={{ textAlign: "center", padding: "12px 0" }}>
                ...
                <p style={{ fontSize: "13px", color: "#7A6E65", lineHeight: 1.5, margin: "0 0 20px 0" }}>
                  {forgotMessage || "If this email is registered, password reset instructions have been sent. Please check your inbox and spam folder."}
                </p>
                <button
                  type="button"
                  onClick={() => {
...
export default function SignInPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: "calc(100vh - 140px)", backgroundColor: "#FAF7F2" }} />}>
      <SignInContent />
    </Suspense>
  );
}
