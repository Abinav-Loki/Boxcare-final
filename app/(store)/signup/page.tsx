...
function SignUpContent() {
  ...
  useEffect(() => {
    ...
    if (error) {
      if (error === "oauth_cancelled") {
        setMessage({ type: "info", text: "Google registration was cancelled." });
      } else if (error === "not_configured") {
        setMessage({
          type: "info",
          text: "Google Sign-Up is not configured yet. Please add your GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to .env.local to enable live Google authentication.",
        });
        ...
        setMessage({
          type: "error",
          text: "A conflict occurred while authenticating with Google. Please sign in or use standard registration.",
        });
      } else if (error === "rate_limit") {
        setMessage({
          type: "error",
          text: "Too many sign-up attempts. Please wait a few minutes before trying again.",
        });
      } else {
        setMessage({
          type: "error",
          text: "Unable to complete Google registration. Please fill in the details below.",
        });
      }
    }
  }, [searchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      setMessage({
        type: "success",
        text: "Account created successfully! Loading your customer profile...",
      });
      setTimeout(() => {
        router.push("/profile");
      }, 800);
      ...
      setMessage({
        type: "error",
        text: result.error || "Unable to register account. Please try again.",
      });
    }
  };
